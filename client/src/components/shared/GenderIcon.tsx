...
// Inlined rather than <img src="/gender-male.svg"> so the glyph takes its colour
// from whatever it sits in. The files these came from hard-code their fills, and
// `color` cannot reach inside an <img>. The transforms flatten the two nested
// translates the source files wrapped their paths in.
const PATHS: Record<Gender, { viewBox: string; transform?: string; d: string }> = {
  male: {
    viewBox: "0 0 20 20",
...
/** Takes its colour from currentColor - see .gender-male / -female / -genderless */
export default function GenderIcon({ gender, className }: Props) {
  const { viewBox, transform, d } = PATHS[gender];
  return (
    <svg className={className} viewBox={viewBox} fill="currentColor" aria-hidden="true">
      <path fillRule="evenodd" transform={transform} d={d} />
    </svg>
  );
}
